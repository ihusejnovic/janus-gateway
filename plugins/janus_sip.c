		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(session->media.audio_rtp_fd != -1 && fds[i].fd == session->media.audio_rtp_fd) {
					...
					if(header->markerbit) {
						JANUS_LOG(LOG_ERR, "[SIP-%s]  before - RTP packet (seq=%"SCNu16", ssrc=%"SCNu32" ts=%"SCNu32", ntohl(ts)=%"SCNu32")\n",
					...
					if(header->markerbit) {
						JANUS_LOG(LOG_ERR,
							  "[SIP-%s]  after - RTP packet (seq=%"SCNu16", ssrc=%"SCNu32" ts=%"SCNu32", ntohl(ts)=%"SCNu32")\n",
							  session->account.username, ntohs(header->seq_number),
							  ntohl(header->ssrc), header->timestamp,
